...
install.packages(c("tseries", "urca", "forecast", "nortest", "MASS", "extRemes", "lubridate", "dplyr", "ggplot2", "kableExtra", "fpp3","evir","extRemes"))
...
crypto<-read.csv("data/Crypto_data.csv")
...
Both tails deviate significantly from the blue line, indicating that Bitcoin is prone to extreme events.
However, the t-distribution exhibits greater tail density compared to the normal distribution, suggesting that the t-distribution allows for a higher likelihood of unexpected, extreme events.
...
```{r, include=TRUE}
rain_df <- read_csv("data/Precipitation_lausanne_full.csv")
...
```{r}
rain_df <- read.csv("data/Precipitation_lausanne_full.csv")
```
...
p_conditional <- 1 - pgpd(precipitation_level_150 - threshold, xi = shape, beta = scale) 
...
```{r}
temp_df <- read.csv("data/Geneva_temperature.csv")
```
...
declustered_data <- extRemes::decluster(summer_df$AvgTemperature, threshold = threshold_temp,
                              method = "runs", 
                              r = 3, 
                              clusterfun = "max")
...
```{r}
raw_data <- read.csv("data/INDEX_US_XNAS_COMP.csv")
...
confidence_levels <- c(0.95, 0.995, 0.999)
...
The **Parametric VaR and ES** rely on assumptions of normality, which may underestimate tail risks, as shown in the earlier analysis (non-normal returns).
While the **Historical VaR and ES** do not make these assumptions, instead relying on actual extreme events from the data, leading to more conservative estimates.

The **Historical POT approach** better captures extreme losses due to its reliance on observed data and focus on the most extreme values.
This makes it more suitable for managing tail risks, especially in volatile markets like the NASDAQ Composite.
Hence, it is more suitable for risk-averse investors who wants to be prepared for worst-case scenarios.

### 3. Models Comparison

```{r}
data <- data.frame(
  Confidence_Level = rep(c("95%", "99.5%"), each = 4),
  Metric = c("BM_Hist_VaR", "BM_Hist_ES", "POT_Hist_VaR", "POT_Hist_ES", 
             "BM_Hist_VaR", "BM_Hist_ES", "POT_Hist_VaR", "POT_Hist_ES"),
  Value = c(0.612, -0.206, -6.175, -7.221, 
            0.272, -0.504, -7.587, -7.736)
)

data <- data %>% filter(!is.na(Value))

ggplot(data, aes(x = Confidence_Level, y = Value, fill = Metric)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Comparison of VaR and ES for Block Maxima and POT Models",
    x = "Confidence Level",
    y = "Value"
  ) +
  # Corrected color mapping
  scale_fill_manual(values = c(
    "BM_Hist_VaR" = "deepskyblue",
    "BM_Hist_ES" = "forestgreen",
    "POT_Hist_VaR" = "seagreen3",
    "POT_Hist_ES" = "cyan3"
  )) +
  theme_minimal() +
...
```{r}
### Assuming you have:
# 1. raw_data with a 'Date' and 'Return' column.
# 2. block_maxima_results containing the VaR estimates from the block maxima approach.
# 3. VaR_POT_df containing the VaR estimates from the POT approach.

# Let's say we choose the 95% VaR threshold from both methods.

# From block_maxima_results (historical or parametric)
# block_maxima_results format is something like:
#   Confidence_Level Historical_VaR Historical_ES Parametric_VaR Parametric_ES
# If we want the historical VaR at 95% (which should be row where Confidence_Level = 0.95):
bm_95_var <- block_maxima_results$Historical_VaR[block_maxima_results$Confidence_Level == 0.95]

# From POT results (VaR_POT_df), format something like:
#   Confidence_Level   VaR          ES (if already computed)
# If Confidence_Level = 0.95:
pot_95_var <- VaR_POT_df$VaR[VaR_POT_df$Confidence_Level == 0.95]

# Now we have two thresholds: bm_95_var and pot_95_var

# Create two portfolios:
# 1) Portfolio_BM: Use the Block Maxima 95% VaR
# 2) Portfolio_POT: Use the POT 95% VaR

# Strategy:
# If today's return < VaR threshold (meaning the loss is greater than VaR), 
# we go to cash the next day (no gain/loss the next day).
# Otherwise, stay fully invested.
# This creates a time series of "portfolio returns" that differ from raw_data returns.

# Make sure returns are in percentage (e.g., -2 means -2%).
# Portfolio return: If invested, portfolio return = raw_data$Return for that day.
# If not invested due to previous day's trigger, portfolio return = 0% that day.

# Initialize indicators
raw_data$Invested_BM <- TRUE  # start fully invested
raw_data$Invested_POT <- TRUE

# We will decide on day t investment based on day t-1 return relative to VaR.
# For the first day, assume we remain invested (no prior info).

for (i in 2:nrow(raw_data)) {
  # Check Block Maxima strategy
  if (raw_data$Return[i - 1] < bm_95_var) {
    # If previous day's return was worse than the VaR,
    # move to cash for day i
    raw_data$Invested_BM[i] <- FALSE
  } else {
    raw_data$Invested_BM[i] <- TRUE
  }
  
  # Check POT strategy
  if (raw_data$Return[i - 1] < pot_95_var) {
    # If previous day's return was worse than the VaR,
    # move to cash for day i
    raw_data$Invested_POT[i] <- FALSE
  } else {
    raw_data$Invested_POT[i] <- TRUE
  }
}

# Now calculate the portfolio returns
# If invested on day i, portfolio return = raw_data$Return[i], else 0.
raw_data$Portfolio_Return_BM <- ifelse(raw_data$Invested_BM, raw_data$Return, 0)
raw_data$Portfolio_Return_POT <- ifelse(raw_data$Invested_POT, raw_data$Return, 0)

# Convert returns to decimal form if they are in percentage
# (If not already done, e.g., -2 means -2%, convert to -0.02)
bm_returns_decimal <- raw_data$Portfolio_Return_BM / 100
pot_returns_decimal <- raw_data$Portfolio_Return_POT / 100

# Compute cumulative returns
bm_cumulative <- cumprod(1 + bm_returns_decimal)
pot_cumulative <- cumprod(1 + pot_returns_decimal)

# Add these to the dataset
raw_data$BM_Cumulative_Return <- bm_cumulative
raw_data$POT_Cumulative_Return <- pot_cumulative

# Finally, you can inspect the cumulative returns:
tail(raw_data[, c("Date", "BM_Cumulative_Return", "POT_Cumulative_Return")])

# Plot the cumulative returns to visualize the performance
plot(
  raw_data$Date, raw_data$BM_Cumulative_Return, 
  type = "l", col = "#27CED7", lwd = 2,
  xlab = "Date", ylab = "Cumulative Return",
  main = "Cumulative Portfolio Returns Under Block Maxima and POT Strategies",
  ylim = range(c(raw_data$BM_Cumulative_Return, raw_data$POT_Cumulative_Return), na.rm = TRUE),
  cex.axis = 0.8, cex.lab = 0.9, cex.main = 0.95  # Adjust text size
)

# Add the second line for POT strategy
lines(raw_data$Date, raw_data$POT_Cumulative_Return, col = "#42BA97", lwd = 2)

# Place the legend outside with smaller font size
legend(
  "topright", 
  inset = c(-0.01, 0), # Moves the legend outside the plot
  legend = c("Block Maxima Strategy", "POT Strategy"),
  col = c("#27CED7", "#42BA97"), lty = 1, lwd = 2, 
  cex = 0.8,  # Makes the legend text smaller
  xpd = TRUE, # Allows the legend to go outside the plot
