...
par(mfrow = c(1, 2))
hist(raw_data$Return, breaks = 30, main = "Histogram of Negative Log Returns", xlab = "Negative Log Returns (%)", col = "#27CED7")
qqnorm(raw_data$Return, main = "Q-Q Plot of Negative Log Returns")
qqline(raw_data$Return, col = "#2683C6")
par(mfrow = c(1, 1))
```
...
**The Historical VaR values indicate that, with 95% confidence, the worst expected daily loss is -2.19%, and with 99% confidence, it is -3.35%. In comparison, the Parametric VaR estimates the worst daily loss at -2.38% for the 95% confidence level and -3.34%** for the 99% confidence level.
...
data.frame(
  ...
  scale_color_manual(values = c(
    "Without Stop-Loss" = "#2683C6", 
    "With Stop-Loss (VaR 95%)" = "#1CADE4", 
    "With Stop-Loss (VaR 99%)" = "#27CED7", 
    "With Stop-Loss (ES 95%)" = "#3E8853", 
    "With Stop-Loss (ES 99%)" = "#62A39F"
  )) +
  labs(
