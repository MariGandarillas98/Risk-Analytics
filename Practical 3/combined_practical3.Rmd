...
gev_var_es <- function(location, scale, shape, confidence_level) {
  ...
  return(list(VaR = -VaR, ES = -ES))  # Return negative values to reflect losses
}
...
# Step 2: Peaks Over Threshold (POT) Model
N <- length(raw_data$Return)
N_exc <- sum(-raw_data$Return > u)  # Number of exceedances
beta <- as.numeric(gpd_fit$par.ests["beta"])
xi <- as.numeric(gpd_fit$par.ests["xi"])
...
pot_results <- data.frame(
  Confidence_Level = confidence_levels,
  POT_VaR = -VaR_POT,  # Convert back to negative returns
  POT_ES = -ES_POT     # Convert back to negative returns
)
